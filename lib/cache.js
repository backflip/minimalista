...
/**
 * Bypass response cache via `?refresh=true` query param
 *
 * @param {object} options
 * @param {object} options.req
 * @returns {boolean}
 */
export function bypassResponseCache({ req }) {
  const requestUrl = new URL(req.url || "", `http://${req.headers.host}`);
  const bypass = requestUrl?.searchParams.get("refresh") === "true";
